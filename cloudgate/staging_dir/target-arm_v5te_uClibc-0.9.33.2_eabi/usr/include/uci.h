...
/*These defines take care of multiple config directory support.
Please refer to path_array for the list of default configuration directories.*/
 ...
 * @top_layer: If false, mark any found sections and options as multi_level
 *
 ...
 */
extern int uci_import(struct uci_context *ctx, FILE *stream, const char *name, struct uci_package **package, bool single, bool top_layer);
 ...
 * @header: hidden, include hidden items with a - in front
 */
extern int uci_export(struct uci_context *ctx, FILE *stream, struct uci_package *package, bool header, bool hidden);
 ...
 * @hidden: also return hidden elements
 *
 ...
 *
 * if hidden is set to true, this function also returns hidden elements. Normally only needed
 * when you plan on setting the element again.
 *
 ...
 */
extern int uci_lookup_ptr(struct uci_context *ctx, struct uci_ptr *ptr, char *str, bool extended, bool hidden);
...
#if 0
/**
 * uci_delete: Hide a section or option
 * @ctx: uci context
 * @ptr: uci pointer
 */
extern int uci_hide(struct uci_context *ctx, struct uci_ptr *ptr);
#endif
...
/**
 * uci_hide_list_item: Hide a specific item from a list
 * @ctx: uci context
 * @ptr: uci pointer
 */
int uci_hide_list_item(struct uci_context *ctx, struct uci_ptr *ptr);
...
/**
 * uci_add_config_path: add a path to the end of the ctx->conf_paths list
 */
extern int uci_add_config_path(struct uci_context *ctx, const char *dir);
...
struct uci_context
{
	...
	/* List of the paths where the config files reside.
	By default this is filled in with the paths in path_array */
	struct uci_list conf_paths;
	...
	/* in case of -H option we need to load hidden sections so we can display them */
	int  load_hidden;
};
