...
BUILD_DIR_UNIT:=$(BUILD_DIR_BASE)/unit
STAGING_DIR_HOST:=$(TOPDIR)/staging_dir/host
STAGING_DIR_UNIT:=$(TOPDIR)/staging_dir/unit
...
STAMP_DIR_UNIT=$(BUILD_DIR_UNIT)/stamp
...
TARGET_LDFLAGS+=-Wl,-rpath-link=$(STAGING_DIR)/usr/lib -Wl,-rpath-link=$(STAGING_DIR)/lib
...
UNITCC:=gcc
UNIT_CFLAGS:=-O2 -I$(STAGING_DIR)/include
UNIT_LDFLAGS:=-L. -L$(STAGING_DIR_UNIT)/lib -L$(STAGING_DIR_UNIT)/usr/lib
...
#oskJanHln bison needs to know where m4 is, it knows, but not when we relocate the SDK, it also needs its pkgdatadir relocated
#but only set M4 when it is build (so a clean make won't have this set, but is not required as bison does not need to be relocatable, and for SDK users it is set and bison is relocatable)
ifneq ($(wildcard $(STAGING_DIR_HOST)/bin/m4),)
  export M4=$(STAGING_DIR_HOST)/bin/m4
  export BISON_PKGDATADIR=$(STAGING_DIR_HOST)/share/bison
endif
#we could also provide an M4 and BISON_PKGDATADIR to the openwrt Makefiles, but as this is unknown upstream probably lost effort
