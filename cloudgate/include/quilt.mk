...
define Build/UnitPatch/Default
	$(if $(QUILT),rm -rf $(UNIT_BUILD_DIR)/patches; mkdir -p $(UNIT_BUILD_DIR)/patches)
	$(call PatchDir,$(UNIT_BUILD_DIR),$(PATCH_DIR),)
	$(if $(QUILT),touch $(UNIT_BUILD_DIR)/.quilt_used)
endef
...
define Kernel/Patch/Default
	...
	# osktomjan: added support to detect patch changes
	cat $(PATCH_DIR)/* | md5sum > $(PKG_BUILD_DIR)/patch-sum
	echo '$(PATCH_DIR)' > $(PKG_BUILD_DIR)/patch-dir
	cat $(GENERIC_PATCH_DIR)/* | md5sum > $(PKG_BUILD_DIR)/genpatch-sum
	echo '$(GENERIC_PATCH_DIR)' > $(PKG_BUILD_DIR)/genpatch-dir
endef
