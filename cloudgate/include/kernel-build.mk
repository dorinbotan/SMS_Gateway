...
pdifftxt = '\
*******************************************************************************\n\
kernel patches have been changed, do a target/linux/clean before proceeding and \n\
reapply your local kernel changes (if any), or build with NOPATCHCHECK=1 if \n\
you want to proceed (e.g. when creating new patches) \n\
*******************************************************************************\
'
...
define BuildKernel
  ...
  checkpatches: FORCE
	@[ "$(NOPATCHCHECK)" == "1" ] || ([ -e "$(PKG_BUILD_DIR)/patch-dir" ] || ( echo -e $(pdifftxt) && exit -1 ))
	@[ "$(NOPATCHCHECK)" == "1" ] || ([ -e "$(PKG_BUILD_DIR)/patch-sum" ] || ( echo -e $(pdifftxt) && exit -1 ))
	@[ "$(NOPATCHCHECK)" == "1" ] || ([ -e "$(PKG_BUILD_DIR)/genpatch-dir" ] || ( echo -e $(pdifftxt) && exit -1 ))
	@[ "$(NOPATCHCHECK)" == "1" ] || ([ -e "$(PKG_BUILD_DIR)/genpatch-sum" ] || ( echo -e $(pdifftxt) && exit -1 ))
	@[ "$(NOPATCHCHECK)" == "1" ] || (cat `cat $(PKG_BUILD_DIR)/patch-dir`/* | md5sum | diff $(PKG_BUILD_DIR)/patch-sum - >/dev/null || (echo -e $(pdifftxt) && exit -2))
	@[ "$(NOPATCHCHECK)" == "1" ] || (cat `cat $(PKG_BUILD_DIR)/genpatch-dir`/* | md5sum | diff $(PKG_BUILD_DIR)/genpatch-sum - >/dev/null || (echo -e $(pdifftxt) && exit -3))
  ...
  compile: $(LINUX_DIR)/.modules checkpatches
