...
LAST_DCFG_DIR=$(TOPDIR)/build_dir
LAST_DCFG=$(LAST_DCFG_DIR)/defconfig_last
%_defconfig: scripts/config/conf prepare-tmpinfo FORCE
	cp $(TOPDIR)/configs/$@ .config
	mkdir -p $(LAST_DCFG_DIR)
	cp $(TOPDIR)/configs/$@ $(LAST_DCFG)_val
	echo $@ > $(LAST_DCFG)_name
	$< -D .config Config.in
...
%::
	@if [ -e $(LAST_DCFG)_val ] && ! diff $(LAST_DCFG)_val $(TOPDIR)/configs/`cat $(LAST_DCFG)_name` 1>/dev/null;\
		then echo "defconfig changed, please do 'make xxx_defconfig' first and reapply your local config changes if needed"; exit -1; fi
